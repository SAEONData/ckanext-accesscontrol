...
anyAuthException = (AccessControlError, OAuth2Error, RequestException)
...
def callback():
    ...
    try:
        ...
        if error:
            ...
            raise AccessControlError(_("Authorization server returned an error: %s %s") % (error, error_description))
...
def _verify_state(state):
    ...
    key = 'oidc_state:' + state
    if key not in redis:
        raise AccessControlError(_("Invalid authorization state"))
...
def _validate_token(token):
    ...
    if not valid:
        raise AccessControlError(_("Invalid access token"))
